...
# Run with: sbatch --requeue --parsable launch_modified_low_latency_test_4_nodes.sh

#SBATCH --account=network_research_swarch
#SBATCH -J network_research_swarch-deepep.modified_low_latency_test
#SBATCH --nodes=4
...
for ((i=1; i<=${NUM_ITERATIONS}; i++)); do
    ...
    export ITER_LOG_OUTPUT="${LOG_DIR}/deepep.modified_low_latency_test_n${SLURM_NNODES}_t${SLURM_NTASKS_PER_NODE}_salloc_${SLURM_JOB_ID}_${LOG_TIMESTAMP}_iter${i}.out"
    export ITER_LOG_ERROR="${LOG_DIR}/deepep.modified_low_latency_test_n${SLURM_NNODES}_t${SLURM_NTASKS_PER_NODE}_salloc_${SLURM_JOB_ID}_${LOG_TIMESTAMP}_iter${i}.err"
    ...
    export PYTHONPATH=${DEEP_EP_DIR}/deepep_custom_install/local/lib/python3.12/dist-packages/deep_ep-1.2.1+61eb6c6-py3.12-linux-x86_64.egg:$PYTHONPATH
