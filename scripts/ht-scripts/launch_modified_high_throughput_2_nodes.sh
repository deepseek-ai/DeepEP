...
# Run with: sbatch --requeue --parsable launch_modified_high_throughput_2_nodes.sh
...
for ((i=1; i<=${NUM_ITERATIONS}; i++)); do
    ...
    --export=ALL,NVSHMEM_IB_SL=${NVSHMEM_IB_SL},MASTER_ADDR=$(hostname),MASTER_PORT=${MASTER_PORT},ENABLE_NCCL_GIN=${ENABLE_NCCL_GIN}   \
    bash -c "
    cd ${WORK_DIR};
