...
public:
    ...
    __device__ __forceinline__ AsymBuffer(void* &gbl_ptr, uint64_t num_elems, int num_ranks,
    ...
    __device__ __forceinline__ AsymBuffer(void** gbl_ptrs, uint64_t num_elems, int num_ranks,
                                          int sm_id = 0, int num_sms = 1, int offset = 0) {
        EP_STATIC_ASSERT(kNumRanks > 1, "");
        num_bytes = num_elems * sizeof(dtype_t);
...
public:
    ...
    __device__ __forceinline__ SymBuffer(void* &gbl_ptr, uint64_t num_elems, int num_ranks,
