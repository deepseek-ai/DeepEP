...
// These modes exist because SGLang changed its per-token-group FP8 kernel
// between 0.5.15.post1 and 0.5.17. The formulas are mathematically
// equivalent, but different divide/multiply and scalar/pairwise conversion
// instructions produce different scale and FP8 bits at rounding boundaries.
// Selecting a compile-time specialization here keeps low-latency dispatch
// bitwise aligned with the training-side SGLang kernel without a device-side
// branch in the hot loop.
enum FP8QuantizationMode {
    FP8_QUANTIZATION_MODE_LEGACY = 0,
    FP8_QUANTIZATION_MODE_SCALAR_RN = 1,
    FP8_QUANTIZATION_MODE_FAST_PAIRWISE = 2,
};

template <bool kUseFP8, bool kUseUE8M0, int kFP8QuantizationMode, int kHidden>
        ...
        for (int token_idx = sm_id; token_idx < num_tokens; token_idx += num_sms) {
            ...
            for (int i = thread_id; i < hidden_bf16_int4; i += num_threads) {
                ...
                if constexpr (kUseFP8) {
                    ...
                    // The aligned modes use SGLang's 1e-10 amax floor. The
                    // legacy mode preserves DeepEP's historical margin.
                    float amax =
                        kFP8QuantizationMode != FP8_QUANTIZATION_MODE_LEGACY and not round_scale
                            ? 1e-10f
                            : kFP8Margin;
                    ...
                    if (round_scale or kFP8QuantizationMode == FP8_QUANTIZATION_MODE_LEGACY) {
                        calculate_fp8_scales(
                            amax, scale, scale_inv, round_scale);
                    } else if constexpr (kFP8QuantizationMode == FP8_QUANTIZATION_MODE_SCALAR_RN) {
                        // Match SGLang 0.5.15.post1's scalar RN sequence.
                        scale_inv = __fdiv_rn(amax, kFinfoAmaxE4M3);
                        scale = __fdiv_rn(1.0f, scale_inv);
                    } else {
                        // Match SGLang 0.5.17's per_token_group_quant
                        // instruction order exactly. The reciprocal forms
                        // are mathematically equivalent, but their FP32
                        // rounding can change both the stored scale and FP8
                        // bytes at bin boundaries.
                        scale_inv = __fmul_rn(
                            amax, 1.0f / kFinfoAmaxE4M3);
                        // SGLang builds per_token_group_quant with
                        // --use_fast_math, so its `/` is __fdividef. This
                        // matters for exact FP8 tie-breaking even when the
                        // stored dequant scale is already identical.
                        scale = __fdividef(kFinfoAmaxE4M3, amax);
                    }
                    ...
                    vec_t int2_value;
                    if (not round_scale and kFP8QuantizationMode == FP8_QUANTIZATION_MODE_SCALAR_RN) {
                        auto fp8_values =
...
void dispatch(void* packed_recv_x, void* packed_recv_x_scales,
    ...
    EP_HOST_ASSERT(FP8_QUANTIZATION_MODE_LEGACY <= fp8_quantization_mode and
                   fp8_quantization_mode <= FP8_QUANTIZATION_MODE_FAST_PAIRWISE);

#define DISPATCH_LAUNCH_CASE(hidden) { \
auto dispatch_func = dispatch<false, false, FP8_QUANTIZATION_MODE_LEGACY, hidden>; \
if (use_fp8 and not use_ue8m0) { \
    if (fp8_quantization_mode == FP8_QUANTIZATION_MODE_SCALAR_RN) \
        dispatch_func = dispatch<true, false, FP8_QUANTIZATION_MODE_SCALAR_RN, hidden>; \
    else if (fp8_quantization_mode == FP8_QUANTIZATION_MODE_FAST_PAIRWISE) \
        dispatch_func = dispatch<true, false, FP8_QUANTIZATION_MODE_FAST_PAIRWISE, hidden>; \
    else \
        dispatch_func = dispatch<true, false, FP8_QUANTIZATION_MODE_LEGACY, hidden>; \
} \
if (use_fp8 and use_ue8m0) \
    dispatch_func = dispatch<true, true, FP8_QUANTIZATION_MODE_LEGACY, hidden>; \
