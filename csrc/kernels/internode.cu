...
__global__ void notify_dispatch(const int* num_tokens_per_rank,
    ...
    if (sm_id == 0) {
        ...
        nvshmemx_qp_quiet_block(NVSHMEMX_PE_ALL, qp_handle, NVSHMEMX_QP_ALL);
        ...
        for (int i = warp_id; i < kNumRDMARanks; i += num_warps) {
            if (i != rdma_rank) {
                nvshmemx_qp_int_put_nbi_warp(rdma_recv_num_tokens_mixed.recv_buffer(rdma_rank),
        ...
        if (thread_id < kNumRDMARanks and thread_id != rdma_rank)
            nvshmemx_qp_quiet(translate_dst_rdma_rank<kLowLatencyMode>(thread_id, nvl_rank), &qp_handle[0], 1);
        __syncthreads();
...
__global__ void __launch_bounds__(((kNumDispatchRDMASenderWarps + 1 + NUM_MAX_NVL_PEERS) * 32), 1)
    ...
    auto rdma_channel_tail = SymBuffer<unsigned long long int, false>(rdma_buffer_ptr, 1, kNumRDMARanks, channel_id, num_channels);
    ...
    if (warp_role == WarpRole::kRDMASender) {
        ...
        for (int dst_rdma_rank = warp_id; dst_rdma_rank < kNumRDMARanks; dst_rdma_rank += kNumDispatchRDMASenderWarps) {
            ...
            if (dst_rdma_rank != rdma_rank) {
                nvshmemx_qp_int_put_nbi_warp(rdma_channel_meta.recv_buffer(rdma_rank),
