...
struct LowLatencyLayout {
    ...
    LowLatencyLayout(void* rdma_buffer, int num_max_dispatch_tokens_per_rank, int hidden, int num_ranks, int num_experts) {
        ...
        size_t logfmt10_bytes_per_combine_msg = hidden * sizeof(nv_bfloat16) / 16 * 10;
        size_t amax_amin_bytes_per_combine_msg = hidden / 256 * 2 * 2 * sizeof(nv_bfloat16);
        size_t num_bytes_per_combine_msg = logfmt10_bytes_per_combine_msg + sizeof(int4) + amax_amin_bytes_per_combine_msg + hidden * sizeof(nv_bfloat16);
        EP_HOST_ASSERT(logfmt10_bytes_per_combine_msg % sizeof(int4) == 0 and amax_amin_bytes_per_combine_msg % sizeof(int4) == 0 and num_bytes_per_combine_msg % sizeof(int4) == 0);
