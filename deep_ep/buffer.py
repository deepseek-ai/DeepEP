...
# SGLang 0.5.15.post1 and 0.5.17 use mathematically equivalent FP8
# quantization formulas, but their scale operations and FP8 conversion
# instructions round differently. Low-latency dispatch must select the
# matching kernel sequence to keep training and rollout bitwise aligned.
_FP8_QUANTIZATION_MODES = {
    'legacy': 0,
    'scalar_rn': 1,
    'fast_pairwise': 2,
}
...
class Buffer:
    ...
    def low_latency_dispatch(self, x: torch.Tensor, topk_idx: torch.Tensor,
                             ...
                             align_fp8_quantization: bool = False,
                             fp8_quantization_mode: Optional[str] = None) -> \
        ...
        Arguments:
            ...
            align_fp8_quantization: compatibility option for SGLang 0.5.15.post1.
                When true, it selects ``fp8_quantization_mode='scalar_rn'``.
            fp8_quantization_mode: FP8 instruction sequence used by the
                non-power-of-two path. ``'legacy'`` preserves DeepEP's default,
                ``'scalar_rn'`` matches SGLang 0.5.15.post1, and
                ``'fast_pairwise'`` matches SGLang 0.5.17. The SGLang modes are
                separate because equivalent formulas can round to different
                FP8 bytes when their CUDA instruction order differs.
            async_finish: the current stream will not wait for the communication kernels to be finished if set.
        ...
        """
        if fp8_quantization_mode is None:
            fp8_quantization_mode = 'scalar_rn' if align_fp8_quantization else 'legacy'
        elif align_fp8_quantization:
            raise ValueError('align_fp8_quantization and fp8_quantization_mode are mutually exclusive')
        if fp8_quantization_mode not in _FP8_QUANTIZATION_MODES:
            choices = ', '.join(_FP8_QUANTIZATION_MODES)
            raise ValueError(f'Unsupported fp8_quantization_mode={fp8_quantization_mode!r}; expected one of: {choices}')
        ...
        packed_recv_x, packed_recv_x_scales, packed_recv_count, packed_recv_src_info, packed_recv_layout_range, event, hook = \
            self.runtime.low_latency_dispatch(x, topk_idx,
                                              ...
                                              _FP8_QUANTIZATION_MODES[fp8_quantization_mode])
