...
lb_prefetch_weights(const int* redundancy_mapping,
                    const ncclDevComm_t nccl_dev_comm, const ncclWindow_t nccl_window,
                    void* workspace,
                    const __grid_constant__ layout::EPWeightList weights) {
    // Indices
    ...
    // Prefetch
    if (ptx::elect_one_sync()) {
        auto iterator = layout::EPWeightIterator<kNumWeights, kNumChunkBytes>(
            gin, smem_layout, weights.weights, workspace_layout.get_lb_chunk_idx_counter_ptr());
        ptx::arrival_phase phase = 0;
        while (iterator.next()) {
            // Load
            ptx::mbarrier_arrive_and_set_tx(mbarrier, iterator.num_chunk_bytes);
            ptx::tma_load_1d(tma_buffer, iterator.src_ptr, mbarrier, iterator.num_chunk_bytes);
            ptx::mbarrier_wait_and_flip_phase(mbarrier, phase);

            // Store and release the shared buffer
            ptx::tma_store_1d(iterator.dst_ptr, tma_buffer, iterator.num_chunk_bytes);
