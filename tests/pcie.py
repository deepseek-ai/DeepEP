...
def test_main(num_tokens: int, num_sms: int, local_rank: int, num_local_ranks: int, num_ranks: int, num_nodes: int, rank: int, buffer: deep_ep.Buffer, group: dist.ProcessGroup):
    ...
    # else :
    #     topk_idx = torch.arange(0, 8, dtype=torch.int64, device='cuda').repeat(num_tokens, 1)
        ...
        for async_mode in (False,):
            for current_x in (x,x_e4m3):
                for with_topk in (False,True):
                    ...
                    # print(f'rank: {rank}, send_rdma_head: {handle[-1]}\n')
                    # print(f'rank: {rank}, topk_idx: {topk_idx}\n') 
                    # print(f'rank: {rank}, recv_topk_idx: {recv_topk_idx}\n') 
                    # print(f'rank: {rank}, recv_topk_weights: {recv_topk_weights}\n')
                    # print(f'rank: {rank}, recv_gbl_channel_prefix_matrix: {handle[2]}\n')
                    # sys.exit()
                    ...
                    if not with_topk:
                        ...
                        event.current_stream_wait() if async_mode else ()
                        recv_x = per_token_cast_back(*recv_x) if isinstance(recv_x, tuple) else recv_x
                        # print(f'-------------w/o topk -------------rank: {rank}, recv_x: {recv_x}\n') 
                    ...
                    # event.current_stream_wait() if async_mode else ()
                    # combined_x = per_token_cast_back(*combined_x) if isinstance(combined_x, tuple) else combined_x
    ...
    for current_x in (x,x_e4m3):
        ...
        for nvl_chunk_size in range(32, 33, 4):
            for rdma_chunk_size in range(4, 33, 4):
                ...
                if local_rank == 0:
                    print(f'[tuning] SMs {num_sms}, NVL chunk {nvl_chunk_size}, RDMA chunk {rdma_chunk_size}: {rdma_send_bytes / 1e9 / t:.2f} GB/s (RDMA) ', flush=True)
        if local_rank == 0:
            print(f'[tuning] Best dispatch ({"FP8" if isinstance(current_x, tuple) else "BF16"}): SMs {best_results[0]}, NVL chunk {best_results[1]}, RDMA chunk {best_results[2]}: {rdma_send_bytes / 1e9 / best_time:.2f} GB/s (RDMA)', flush=True)
            print('', flush=True)
