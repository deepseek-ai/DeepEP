...
def enumerate_ep_modes(args: argparse.Namespace):
    for expert_alignment in (128, 1):
        for use_fp8_dispatch in ((1, ) if args.fp8_dispatch_only else (1, 0)):
            for num_bias in (0, 1, 2):
                for with_previous_event in (0, 1):
                    for async_with_compute_stream in ((1, ) if args.defer_epilogue else (0, 1)):
                        for allocate_on_comm_stream in ((1, ) if with_previous_event else (0, 1)):
                            yield (expert_alignment, use_fp8_dispatch, num_bias,
                                   with_previous_event, async_with_compute_stream, allocate_on_comm_stream)


def launch(buffer: deep_ep.EPBuffer, name: str,
           with_previous_event: int, async_with_compute_stream: int,
           defer_epilogue: int, params: dict):
    ...
    if defer_epilogue:
        return *result.current_stream_wait(), result
...
def test_dispatch_combine(buffer: deep_ep.EPBuffer, args: argparse.Namespace):
    ...
    for (expert_alignment, use_fp8_dispatch, num_bias,
        ...
        dist_print(f' > Testing with '
                   f'{expert_alignment=}, {use_fp8_dispatch=}, {num_bias=}, '
        ...
        recv_x, recv_topk_idx, recv_topk_weights, handle, dispatch_event = \
            launch(buffer, 'dispatch', with_previous_event, async_with_compute_stream, args.defer_epilogue, dispatch_args)
        ...
        cached_recv_x, cached_recv_topk_idx, cached_recv_topk_weights, cached_handle, cached_dispatch_event = \
            launch(buffer, 'dispatch', with_previous_event, async_with_compute_stream, args.defer_epilogue, cached_dispatch_args)
        ...
        cached_expanded_recv_x, _, cached_expanded_recv_topk_weights, _, _ = \
            launch(buffer, 'dispatch', with_previous_event, async_with_compute_stream, args.defer_epilogue, cached_expanded_dispatch_args)
        ...
        combined_x, combined_topk_weights, combine_event = \
            launch(buffer, 'combine', with_previous_event, async_with_compute_stream, args.defer_epilogue, combine_args)
        ...
        reduced_combined_x, reduced_combined_topk_weights, reduced_combine_event = \
            launch(buffer, 'combine', with_previous_event, async_with_compute_stream, args.defer_epilogue, reduced_combine_args)
        ...
        if not args.skip_perf_test:
            ...
            # Synchronize
            # To prevent later dispatch's results to affect other ranks' previous correctness tests
            # Another way to do this is, update the previous event
            torch.cuda.synchronize()
        ...
        if not args.skip_check:
            # Handles retain the original routing indices
            assert handle.topk_idx is topk_idx
            assert cached_handle.topk_idx is topk_idx
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Test EP kernels')
    ...
    parser.add_argument('--defer-epilogue', type=int, default=1, help='Whether to defer epilogues in correctness tests')
